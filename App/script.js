let bars = [];
const def = "#00dbfd", chng = "purple", finished = "green", selected = "yellow";

window.onload = setup();
// why is this async function
async function setup() {
	let b = document.getElementById("bars");
	let d = document.getElementById("delay");

	// updates number and speed
...
// makes the number bar, speed bar and sorting algo buttons non-interactive
function Disable_The_Input() {
	// disable all elements with input tag
	let x = document.getElementsByTagName("input");
	for (let i = 0; i < x.length; i++)
		x[i].disabled = true;
	// returning delay value
	return parseInt(document.getElementById("delay").value);
}


function Finished_Sorting() {
	// change all bars colour to finished 
	let x = document.getElementsByClassName("bar");
	for (let i = 0; i < x.length; i++)
		x[i].style.backgroundColor = finished;
	// enable all elements with input tag again
...
// generating n individual bars
function generateBars(n = -1) {
	bars = [];
	let container = document.getElementById("container");
	n = n < 0 ? Math.random() * 20 : n;
	for (let i = 0; i < n; i++) {
		// sets class = bar, id = i and height using style attribute = random number between 2 and 100%
		bars.push('<div class="bar" id="' + i + '" style="height:' + Math.floor(2 + Math.random() * 98) + '%"></div>');
	}
	// joins the bar with a space in between
...
async function SelectionSort() {
	...
	for (let i = 0; i < bars.length; i++) {
		...
		for (let j = i + 1; j < bars.length; j++) {
			...
			// changes the colour from chng to def after delay/5.0 ms
		...
		// swaps mn_ind to i after delay ms
		await Sleep(delay);
		...
		// displays a new string of bars after swapping
		container.innerHTML = bars.join('');
		await Sleep(delay);
